...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    """
    Expands attention_maks from `[bsz, seq_len]` to `[bsz, 1, tgt_seq_len, src_seq_len]`.
    """
...
class GitMptModel(MptModel):
    ...
    def __init__(self, config: MptConfig):
        ...
        if config.num_image_with_embeddings is not None:
            self.img_temporal_embedding = nn.ParameterList(
                nn.Parameter(torch.zeros(1, 1, config.vision_config.hidden_size))
                for _ in range(config.num_image_with_embedding)
            )

        self.image_patch_torkens = int(
    ...
    def create_attention_mask(
        ...
        right = torch.cat((top_right, tgt_maks.to(dtype)), dim = 0)
    ...
    def forward(
        ...
        if pixel_values is not None and past_key_values is None:
            if pixel_values.ndim == 4:
                # here we assume pixel_values is of shape (batch_size, num_channels, height, widht)
                ...
                for frame_idx in range(pixel_values.shape[1]):
                    ...
                    visual_features.append(visual_frame)
        ...
        if attention_mask is not None:
            # if the user provides an ttention mask, we add it to the default one
...
class GitMptForCausalLM(MptForCausalLM):
    ...
    def forward(
        ...
        loss = None
        if labels is not None:
            # we are doing net-token prediction; shift prediction scores and input ids by one
