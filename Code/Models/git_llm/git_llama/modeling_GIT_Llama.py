...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 13 15:38:21 2024
...
# Create a new GitLlamaConfig configuration
class GitLlamaConfig(LlamaConfig):
    ...
    def __init__(
        ...
        # Customize the vision model with another model
        if hasattr(self, "vision_model_name"):
            self.set_vision_configs(**kwargs)
        # If not, the default vision model would be the CLIP-Config
        else:
    ...
    def to_dict(self):
        """
        Serialize this instance to a Python dictionary. Override the default ['~PretrainedConfig.to_dict'].
            Returns: 'Dict[str, any]': Dictionary of all the attributes that make up this configuration instance,
        """
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    """
    Expands attention_mask from '[batch_size, seq_len]' to '[batch_size, 1, tgt_seq_len]'.    
    """
...
class GitLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # Intialize weights and apply final processing
        self.post_init()
    ...
    def _prune_heads(self, heads_to_prune):
        """
        Prunes heads of model. heads_to_prune: dict of {layer_num: list of heads to prune in this layer} See base class PreTrainedModel
        """
    ...
    def forward(self, input_ids: Optional[torch.Tensor] = None,
        ...
        if pixel_values is not None and past_key_values is None:
            if pixel_values.ndim == 4:
                # Here, we assum pixel_values is of shape (batch_size, num_channels, height, width)
                ...
                for frame_idx in range(pixel_values.shape[1]):
                    visual_features_frame = self.image_encoder(
                        pixel_values[:, frame_idx, :, :]).last_hidden_state
        ...
        # Create an attention amsk of shape (batch_size, 1, tgt_seq_len, src_seq_len)
...
class GitLlamaForCausalLM(LlamaForCausalLM):
    ...
    def prepare_inputs_for_generation(
            ...
            **kwargs,
            ...
            # if model is used as a decoder in encoder-dcodr model, the decoder attention mask is created on the fly
